...
const INTEGRITY_CHECKSUM = '02f4ad4a2797f85668baf5a7f0dd74a1'
...
function sendToClient(client, message, timeout = 1000) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      reject(new Error('Failed to receive a response'))
    }, timeout)
  })
}

function respondWithMock(response) {
  // Setting response status code to 0 is not a valid way to respond with a mocked response
  // in workbox-strategies caching layer. Status 0 is reserved for Network errors.
  const status = response.status === 0 ? 200 : response.status
  const headers = Object.keys(response.headers).reduce((headers, key) => {
  ...
  Reflect.defineProperty(mockResponse, IS_MOCKED_RESPONSE, {
    value: true,
    enumerable: true,
  })
