    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F8F9FB]">
        <div className="text-center space-y-4">
          ...
          <Button onClick={() => navigate(-1)} variant="default" className="h-9">
      ...
      <div className="max-w-[1400px] mx-auto space-y-6">
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-lg font-medium text-muted-foreground">
                    ...
                    <span>{job.organization?.name || "Unknown Organization"}</span>
                  </div>
                </div>
                
                {/* Status Badges */}
                <div className="flex items-center gap-2">
                   <Badge variant="secondary" className="capitalize rounded-lg px-3 py-1 font-medium bg-muted text-muted-foreground">
                    {job.type}
                  </Badge>
                  <Badge
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase text-muted-foreground">Salary Range</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase text-muted-foreground">Job Type</p>
                    <p className="font-semibold text-foreground capitalize">{job.type}</p>
                  </div>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card className="rounded-3xl border-border bg-card shadow-none min-h-[500px]">
              <CardHeader className="border-b border-border pb-6 px-8 pt-8">
                <CardTitle className="text-xl font-bold text-foreground">About the Role</CardTitle>
              </CardHeader>
              <CardContent className="p-8">
                <Markdown
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <Card className="rounded-3xl border-border bg-card shadow-none">
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold text-foreground">{job.applications}</span>
                </div>
            ...
            <Card className="rounded-3xl border-border bg-card shadow-none">
              <CardHeader className="border-b border-border pb-4 px-6 pt-6">
                <CardTitle className="text-lg font-bold text-foreground">Keywords</CardTitle>
              </CardHeader>
              <CardContent className="p-6">
                {job.keywords && job.keywords.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {job.keywords.map((kw) => (
                      <Badge key={kw} variant="outline" className="rounded-md border-border text-muted-foreground hover:bg-muted font-normal">
                        {kw}
                      </Badge>
                    ))}
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground italic">No keywords attached.</p>
                )}
              </CardContent>
            </Card>
          </div>
