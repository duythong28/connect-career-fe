...
const CopyableId = ({ id }: { id: string | null | undefined }) => {
  ...
  if (!id) return <span className="text-muted-foreground italic text-xs">Not available</span>;
  ...
  return (
    <div className="flex items-center gap-2 bg-muted/30 px-2 py-1.5 rounded-lg w-fit max-w-full border border-border/50 group hover:border-primary/30 transition-colors">
      <span className="font-mono text-[11px] text-muted-foreground truncate max-w-[140px] sm:max-w-[200px]" title={id}>
        {id}
      </span>
      <Button
        variant="ghost"
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#F8F9FB] gap-4">
        ...
        <p className="text-muted-foreground font-medium">Loading transaction details...</p>
      </div>
    );
  }
    ...
    return (
      <div className="p-6 bg-[#F8F9FB] min-h-screen flex items-center justify-center">
        <div className="bg-card border border-border rounded-3xl p-8 max-w-md text-center shadow-sm">
          ...
          <p className="text-muted-foreground mb-6">{"We couldn't find the transaction you're looking for."}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#F8F9FB] p-4 md:p-8 animate-fade-in">
      ...
      <div className="max-w-5xl mx-auto mb-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Transaction Receipt</h1>
            <p className="text-sm text-muted-foreground mt-1">
              Processed on {new Date(transaction.createdAt).toLocaleString('en-US', { 
                dateStyle: 'long', 
                timeStyle: 'short' 
              })}
            </p>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-card border border-border rounded-3xl p-8 relative overflow-hidden">
            ...
            <div className="flex items-center gap-5 mb-8">
              ...
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Amount Total</p>
                <p className={cn(
                  "text-3xl font-bold",
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12 pt-6 border-t border-dashed border-border">
              <div className="space-y-5">
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground block mb-2">Internal Reference</label>
                  <CopyableId id={transaction.id} />
                </div>
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground block mb-2">Payment Provider Ref</label>
                  <CopyableId id={transaction.metadata?.paymentTransactionId || transaction.relatedPaymentTransactionId} />
                </div>
              </div>

              <div className="space-y-5">
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground block mb-2">Transaction Type</label>
                ...
                <div>
                  ...
                  <p className="text-sm text-foreground font-medium italic">"{transaction.description || "No notes available for this transaction."}"</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-3xl p-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="text-lg font-bold text-foreground">Balance Adjustment</h3>
            </div>
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 rounded-2xl bg-[#F8F9FB] border border-border">
              <div className="text-center md:text-left">
                <p className="text-[10px] font-bold uppercase text-muted-foreground mb-1">Previous Balance</p>
                <p className="text-lg font-bold">{transaction.balanceBefore} <span className="text-sm font-normal text-muted-foreground">{transaction.currency}</span></p>
              </div>
              ...
              <div className="text-center md:text-right">
                <p className="text-[10px] font-bold uppercase text-primary mb-1">New Balance</p>
                <p className="text-lg font-bold text-primary">{transaction.balanceAfter} <span className="text-sm font-normal opacity-70">{transaction.currency}</span></p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-card border border-border rounded-3xl p-6 shadow-sm">
            <div className="flex items-center gap-2 mb-5">
              ...
              <h3 className="text-lg font-bold text-foreground">Account Source</h3>
            </div>
            <div className="space-y-5">
              <div>
                <label className="text-[10px] font-bold uppercase text-muted-foreground block mb-2">Associated Wallet</label>
                <CopyableId id={transaction.walletId} />
              </div>
              <div>
                <label className="text-[10px] font-bold uppercase text-muted-foreground block mb-2">User Reference</label>
                <CopyableId id={transaction.userId} />
              </div>
              
              <div className="pt-4 mt-2 border-t border-border">
                <div className="flex justify-between items-center py-2">
                  <span className="text-sm text-muted-foreground">Current Credit</span>
                  <span className="text-sm font-bold text-foreground">{transaction.wallet.creditBalance} {transaction.wallet.currency}</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-sm text-muted-foreground">Auto Top-up</span>
                  <Badge variant="outline" className={cn(
                    "text-[10px] font-bold px-2 py-0",
                    transaction.wallet.autoTopUpEnabled ? "text-green-600 border-green-200 bg-green-50" : "text-muted-foreground"
                  )}>
                    {transaction.wallet.autoTopUpEnabled ? "ACTIVE" : "INACTIVE"}
                  </Badge>
                </div>
              </div>
            </div>
            ...
            <div className="bg-primary text-primary-foreground rounded-3xl p-6 shadow-md">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-start border-b border-primary-foreground/20 pb-3">
                  <span className="text-xs font-medium opacity-80">Paid Amount</span>
                  <div className="text-right">
                    <p className="font-bold">{transaction.metadata.originalAmount} {transaction.metadata.originalCurrency}</p>
                    <p className="text-[10px] opacity-70">Rate: {transaction.metadata.exchangeRate}</p>
                  </div>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs font-medium opacity-80">Final Credit</span>
                  <span className="font-bold">{transaction.metadata.convertedAmount} {transaction.currency}</span>
                </div>
              </div>
            </div>
          )}
