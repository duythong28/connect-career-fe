    ...
    <div className={cn("prose prose-sm max-w-none dark:prose-invert text-muted-foreground leading-relaxed", className)}>
      <ReactMarkdown
        ...
        components={{
          ...
          ul: ({ children }) => <ul className="list-disc list-outside ml-4 mb-3 space-y-1 text-sm">{children}</ul>,
          a: ({ href, children }) => (
...
const MetaItem = ({ label, value, icon: Icon }: { label: string; value: any; icon?: any }) => (
...
const BackOfficeCompanyPage = () => {
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Status Updated", description: "Company status synchronized." });
      refetch();
    },
  });

  if (isLoading) return <div className="p-8 text-center text-muted-foreground animate-fade-in">Loading company details...</div>;
  if (!data) return <div className="p-8 text-center text-foreground font-bold animate-fade-in">Company not found.</div>;
      ...
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <Card className="bg-card border-border rounded-3xl shadow-none p-6 md:p-8">
          <div className="flex flex-col md:flex-row gap-8 items-start">
            <Avatar className="h-24 w-24 rounded-2xl border border-border shadow-sm shrink-0">
              ...
              <AvatarFallback className="bg-secondary text-primary text-4xl font-bold">{org.name?.charAt(0)}</AvatarFallback>
            </Avatar>
            ...
            <div className="flex-1 w-full space-y-4">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    {org.isVerified && <ShieldCheck size={20} className="text-primary" />}
                    <Badge className={cn("text-[10px] font-bold uppercase rounded-md px-2 py-0.5", org.isActive ? "bg-[hsl(var(--brand-success))]" : "bg-destructive")}>
                      {org.isActive ? "Active" : "Inactive"}
                    </Badge>
                  </div>
                  <p className="text-muted-foreground text-sm">{org.shortDescription}</p>
                </div>
                <Button
              ...
              <div className="flex flex-wrap gap-6 border-t border-border pt-4">
                <MetaItem label="Location" value={`${org.city}, ${org.country}`} icon={MapPin} />
                <MetaItem label="Website" value={<a href={org.website} className="text-primary hover:underline">{org.website}</a>} icon={Globe} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            ...
            { label: "Applicants", val: stats.totalApplications, icon: MessageSquare },
            ...
            <Card key={i} className="bg-card border-border rounded-2xl p-4 shadow-none">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-secondary rounded-xl text-primary"><s.icon size={18} /></div>
                <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <Card className="lg:col-span-8 bg-card border-border rounded-3xl p-6 md:p-8 shadow-none">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-lg font-bold text-foreground">Detailed Information</h2>
            </div>
            
            <div className="space-y-8">
              {org.longDescription && (
                <div className="text-sm">
                  <span className="text-xs font-bold uppercase text-muted-foreground mb-3 block">About Company</span>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-y-8 gap-x-4 pt-8 border-t border-border">
                <MetaItem label="Type" value={formatEnumValue(org.organizationType)} />
                <MetaItem label="Size" value={formatEnumValue(org.organizationSize)} />
                <MetaItem label="Employees" value={org.employeeCount} />
                <MetaItem label="Founded" value={org.foundedDate ? new Date(org.foundedDate).getFullYear() : "N/A"} />
                <MetaItem label="Industry ID" value={org.industryId} />
                <MetaItem label="Headquarters" value={org.headquartersAddress} />
              </div>
            </div>
          </Card>
          ...
          <Card className="lg:col-span-4 bg-card border-border rounded-3xl p-6 shadow-none">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-lg font-bold text-foreground">Work Culture</h2>
            </div>
            
            <div className="space-y-6">
              <div>
                <span className="text-[10px] font-bold uppercase text-muted-foreground mb-2 block">Working Days</span>
                <div className="flex flex-wrap gap-1.5">
                  {org.workingDays?.map(day => (
                    <Badge key={day} variant="secondary" className="rounded-lg text-[10px] font-bold capitalize border-transparent">
                      {day.toLowerCase()}
                    </Badge>
                  ))}
                </div>
              ...
              <div>
                <span className="text-[10px] font-bold uppercase text-muted-foreground mb-2 block">Schedules</span>
                <div className="flex flex-wrap gap-1.5">
                  {org.workScheduleTypes?.map(type => (
                    <Badge key={type} className="rounded-lg bg-primary/10 text-primary border-transparent text-[10px] font-bold">
              ...
              <div className="pt-6 border-t border-border">
                <MetaItem label="Overtime Policy" value={formatEnumValue(org.overtimePolicy)} icon={Calendar} />
              </div>
            </div>
          </Card>
        </div>
