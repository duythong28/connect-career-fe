...
export default function AIAgentChatPageV2() {
  ...
  const formatMessages = useCallback((apiMessages: ConversationMessage[] | Record<string, unknown>[]): Message[] => {
  ...
  const renderAgentBadge = (agentName?: string, intent?: string) => {
    ...
    const getColors = (name: string) => {
      if (name.includes("Job")) return "bg-emerald-100 text-emerald-700 border-emerald-200";
      if (name.includes("Analysis")) return "bg-purple-100 text-purple-700 border-purple-200";
      return "bg-blue-50 text-blue-700 border-blue-200";
    };

    return (
      <div className="flex items-center gap-2 mb-2">
        <Badge className={cn("text-[10px] uppercase tracking-wider font-bold", getColors(agentName))}>
          {agentName}
        </Badge>
        {intent && (
              ...
              >
                <div className="flex items-start gap-3">
                  <MessageSquare className={cn(
                    ...
                    currentSessionId === session.id ? "text-[#0EA5E9]" : "text-gray-400"
                  )} />
                  <div className="flex-1 min-w-0">
                    <div className={cn(
                      "font-bold text-sm mb-1 truncate",
                      currentSessionId === session.id ? "text-[#0EA5E9]" : "text-gray-900"
                    )}>
                      {session.title}
                    </div>
                    {session.lastMessage && (
                    ...
                    <div className="text-[10px] text-gray-400">
                      {session.timestamp.toLocaleDateString()} • {session.messageCount || 0} messages
                    </div>
                  </div>
                </div>
              </button>
              ...
              {sessions.map((session) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-bold text-sm mb-1 truncate">{session.title}</div>
                      {session.lastMessage && (
            ...
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center">
                ...
                <p className="text-gray-500 max-w-2xl mb-10 text-base">
                  Ask about job opportunities, get your CV analyzed, or request career strategy advice.
                </p>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-3xl">
                  {[
                    { icon: <Briefcase className="w-5 h-5"/>, text: "Find Senior React jobs in NY", prompt: "I'm looking for senior React developer jobs in New York" },
                    { icon: <FileText className="w-5 h-5"/>, text: "Analyze my resume", prompt: "Can you analyze my CV and suggest improvements?" },
                    { icon: <DollarSign className="w-5 h-5"/>, text: "Compare job offers", prompt: "Help me compare these job offers" },
                    { icon: <BrainCircuit className="w-5 h-5"/>, text: "Career path advice", prompt: "What skills should I learn next for my career?" },
                  ].map((item, i) => (
                    <button 
                      key={i}
                      onClick={() => {
