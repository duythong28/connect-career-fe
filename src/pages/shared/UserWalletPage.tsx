...
const StatCard = ({ icon: Icon, title, amount, currency, color, subtitle, iconBg, amountColor }) => (
    <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm transition-all hover:shadow-md group">
        ...
        <div className="flex items-end">
            ...
            {currency && <span className="text-sm text-gray-500 ml-2 mb-0.5">{currency}</span>}
        </div>
...
const UserWalletPage = () => {
  ...
  const topUpMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      // Using a toast style consistent with the design language
      ...
      {walletData?.statistics && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <StatCard
            icon={CreditCard}
            title="Total Spent"
            amount={walletData.statistics.totalSpent.total
                ? formatCurrency(walletData.statistics.totalSpent.total)
                : "N/A"}
            currency={walletData.statistics.totalSpent.total ? walletData.currency : ''}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <span className="text-[10px] bg-gray-100 px-2 py-1 rounded-md text-gray-600 font-medium">
                        Before: {formatCurrency(transaction.balanceBefore)} {walletData.currency}
                      </span>
                      <span className="text-[10px] bg-gray-100 px-2 py-1 rounded-md text-gray-600 font-medium">
                        After: {formatCurrency(transaction.balanceAfter)} {walletData.currency}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4 mb-6">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            setPaymentMethod(
                                paymentMethods[val.target.value as keyof typeof paymentMethods][0].value
                            );
                            }}
