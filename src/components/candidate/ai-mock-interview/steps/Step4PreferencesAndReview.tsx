    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <label className="flex items-start p-5 rounded-2xl border-2 border-gray-200 hover:border-indigo-300 cursor-pointer transition-all">
          <input
            type="checkbox"
            checked={config.realtimeScoring}
            onChange={(e) => setConfig({ ...config, realtimeScoring: e.target.checked })}
            className="w-5 h-5 text-indigo-600 rounded accent-indigo-600 mt-1"
          />
          <div className="ml-4 flex-1">
            <div className="flex items-center mb-1">
              <BarChart3 className="w-5 h-5 mr-2 text-purple-600" />
              <span className="font-bold text-gray-900">Real-time Feedback</span>
            </div>
            <p className="text-sm text-gray-600">Get instant AI feedback after each answer.</p>
          </div>
        </label>
      </div>
